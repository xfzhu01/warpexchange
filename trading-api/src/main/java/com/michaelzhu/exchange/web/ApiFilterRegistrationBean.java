            ...
            if (userId == null) {
                ...
                try (UserContext ctx = new UserContext(userId)) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("processing request with user context: userId={}", userId);
                    }
                    chain.doFilter(request, response);
                } catch (Exception e) {
                    logger.error("error processing request for user {}: {}", userId, e.getMessage(), e);
                    sendErrorResponse(response, new ApiException(ApiError.INTERNAL_SERVER_ERROR, null, "Internal server error"));
                }
            }
        }
